...
var mongo={
    ...
    getVendorListing : function(query,callback){
        ...
        /* No need to pass graphnode to app */
        var projection={graphnode_id:0};
    ...
    checkListingMatches:function(listing_id,vendor_id,callback) {
        ...
        pricepls_db.collection(COLLECTION_LISTING).find(query).toArray(function (err, listing) {
            ...
            if (err)
                ...
                callback(null, {exists:true,graphnode_id:listing[0].graphnode_id});
            } else {
                callback(null, {exists:false,graphnode_id:listing[0].graphnode_id});
            }
    ...
    updateListingGraphNodeID:function(listing_id,node_id,callback){

        var query={
            listing_id:listing_id
        };
        var params = {
            $set:{graphnode_id:node_id}
        };
        pricepls_db.collection(COLLECTION_LISTING).update(query,params,{upsert:true},function(err,status){
