...
function populateCities() {
    ...
    mongodb.getOfferCronSettings(cronid, function (err, settings) {
        ...
        var plan_one_service_ids = [1];
        var plan_two_service_ids = [2];
        var plan_three_service_ids = [3];
        var plan_four_service_ids = [4];
        var plan_five_service_ids = [5];
        ...
        async.parallel([
                ...
                mysql.getNewnModifiedCities(minutes, function (err, cities) {
                    ...
                    if (err)
                        ...
                        async.forEach(cities, function (city, callback) {
                            ...
                            var qs = "address=" + city.name + "&key=" + geocode_key;
                                ...
                                if (!err) {
                                    ...
                                    if (data != null) {
                                        ...
                                        if (result.length > 0) {
                                            ...
                                            var info = result[0].geometry.location;
                                            each_city.lat = info.lat;
                                            each_city.lng = info.lng;
            ...
            async.forEach(cities_obj, function (city, callback) {
                ...
                city_query.lat = city.lat;
                city_query.lng = city.lng;
                //city_query.services=[];
                        ...
                        if (err)
                            ...
                            if (listings.count > 0) {
                                ...
                                async.forEach(listings, function (each_listing, callback) {
                                    ...
                                    var listing_category_name = each_listing.category;
                                    if (listing_category_name.toLowerCase() === "Event Organizer") {
                                        listing_category_name = "events";
                                    }
                                    ...
                                    if (active_subcategories.indexOf(listing_subcategory) === -1) {
                                        ...
                                        eachService.cat_name = listing_category_name;
                                    ...
                                    active_categories.forEach(function (cat) {
                                        ...
                                        if (plan_four_service_ids.indexOf(cat) !== -1) {
                                            if (plans_enabled.indexOf(4) === -1)
                                                plans_enabled.push(4);

                                        }
                                        if (plan_five_service_ids.indexOf(cat) !== -1) {
                                            if (plans_enabled.indexOf(5) === -1)
                                                plans_enabled.push(5);
                                    ...
                                    console.log('------active servcies-----' + JSON.stringify(active_services));
                                    active_services.forEach(function (ser) {

                                        console.log("-------service id-----" + ser.id + '---name (service  )---' + ser.name + '---' + ser.cat_name);

                                        if (!previous_serv) {
                                            each_one.service_id = ser.id;
                                            each_one.service_name = ser.cat_name.toLowerCase();
                                    ...
                                    if (service_enabled.length == 0 || plans_enabled == "") {
                            ...
                            if (eachVendor.pp_price) {
                                ...
                                mongodb.getEachVendorListing(query, function (err, lisitng) {
                                    ...
                                    if (lisitng !== null) {
                                        ...
                                        var head_count_enabled = lisitng.head_count_enabled || false;
                                        eachVendor.pp_price.forEach(function (each_room_pprice, index) {

                                            if (each_room_pprice.price != null && each_room_pprice.price != "") {
                                        ...
                                        if (lisitng.extra_images)
                                            ...
                                            var request_img = "/0000/picture_not_available.png";
                                        ...
                                        var params = {
                                            $set: {
                                                ...
                                                'notified_vendors.$.head_count_enabled': head_count_enabled
                                            }
                                        }
                                ...
                                mongodb.getUserGCMToken(each_request.user_id, function (err, token) {
                                    ...
                                    if (err)
                                        ...
                                        if (token != undefined) {
                                            ...
                                            var message = {
                                                'id': each_request.request_id,
                                                'userId': each_request.user_id,
                                                'token': token
                                            }
                                            logger.info("notification message" + JSON.stringify(message));
                ...
                if (listings !== null) {
                    ...
                    async.forEach(listings, function (eachListing, callback) {
                        ...
                        if (eachListing.images.length > 0) {
                            ...
                            default_images.key = "picture_not_available.png";
                        }

                        logger.info("listing.images ", "image to be updated " + image);

                        console.log("default image " + default_image + " no other image " + no_other_image);

                        if (image !== undefined) {


                            if (!default_image && no_other_image) {

                                var modified_images = {
                                    "booking_details_url": util.getModifiedImage(image, eachListing.listing_number, '150', '150'),
                                    "request_list_url": util.getModifiedImage(image, eachListing.listing_number, '300', '150'),
                                };

                            } else if (default_image) {

                                var modified_images = {
                                    "booking_details_url": util.getModifiedImage(default_images.key, '0000', '150', '150'),
                                    "request_list_url": util.getModifiedImage(default_images.key, '0000', '300', '150'),
                                };
                            } else {

                                var modified_images = {
                                    "booking_details_url": util.getModifiedImage(image, '0000', '150', '150'),
                                    "request_list_url": util.getModifiedImage(image, '0000', '300', '150'),
                                };
