                        ...
                        if (err)
                            ...
                            if (listings.count > 0) {
                                ...
                                async.forEach(listings, function (each_listing, callback) {
                                    ...
                                    //        previous_sub = ser.value;
                                    //        each_one.sub_services={};
                                    //        each_one.sub_services.subservice_id=ser.value;
                                    //        each_one.sub_services.subservice_name=ser.name;
                                    //
                                    //    }else if(previous_serv == ser.id){
                                    //
                                    ...
                                    //        each_one = {};
                                    //        each_one.sub_services={};
                                    //        each_one.sub_services.service_id = ser.id;
                                    //        each_one.sub_services.subservice_id=ser.value;
                                    //        each_one.sub_services.subservice_name=ser.name;
                                    //        previous_serv = ser.id;
                                    //
                                    ...
                                    if (err)
                                        return callback(err);
                                    logger.log("debug","city query"+JSON.stringify(city_query));
                                    var query = {
                                        id: city_query.id
                                    }
                            ...
                            if (eachVendor.pp_price) {
                                ...
                                mongodb.getEachVendorListing(query, function (err, lisitng) {
                                    ...
                                    if (lisitng !== null) {
                                        ...
                                        logger.log("debug","requestFormatting "+JSON.stringify(room_types));
                                        room_types.forEach(function (roomtype, index) {
                                            ...
                                            if (each_room_pprice !== undefined) {
                                                ...
                                                if (category_id == 1) {
                                                    ...
                                                    //var per_room_guests = constants.constants.guests_per_room;
                                                    //var total_rooms = Math.floor(no_of_guest/per_room_guests);
                                                    final_amount = room_acutal_price * no_of_guest;
                                                    eachVendor.pp_price[index]['base_amount'] = final_amount;
                    ...
                    async.forEach(listings, function (eachListing, callback) {
                        ...
                        if(eachListing.images.length >0){
                            ...
                            if(eachListing.images[0].name === "default" && eachListing.images.length >1){
                                image = eachListing.images[1].url;
                            }else{
                                image = eachListing.images[0].url;
                            }
                        ...
                        if (image !== undefined) {
                            ...
                            var image_name = image.split('/').pop();
                            var image_extension = image_name.split('.');
                            var actual_name = image_extension[0];
                            var actual_extension = image_extension[1];
                            util.downloadS3File("listing.images",image,__dirname +'/../s3_files/'+actual_name+'.'+actual_extension,function(err,data){

                                crop.changeImage(actual_name+'.'+actual_extension,image_name,actual_extension,eachListing.listing_number,function(modified_images){
